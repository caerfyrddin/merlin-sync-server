...
namespace Caerfyrddin\MerlinSyncServer\Core\Session;

use Caerfyrddin\MerlinSyncServer\Core\App;
use Caerfyrddin\MerlinSyncServer\Shared\Domain\User\User;
...
class SessionManager
{
     ...
     *                  HTTP.
     */
    public function requireLoggedIn(bool $api = false): void
    {
        if (! $this->isLoggedIn()) {
            if (! $api) {
                CommonHelper::ddl(null, null);
     ...
     *                  HTTP.
     */
    public function requireNotLoggedIn(bool $api = false): void
    {
        if ($this->isLoggedIn()) {
